...
在[3.1.6节](dl/Chapter03/Ch03.1_LinearRegression)内容中我们介绍到，神经网络中多次线性组合后的输出结果仍旧只是原始特征的线性组合它并没有增加模型的复杂度。为了增加模型的表达能力需要对每一层的输出结果通过一个激活函数（Activation Function）来进行一次非线性变换然后再将其作为下一层网络的输入。在深度学习中，常见的激活函数包括Sigmoid、Tanh、ReLU和LeakyReLU等，下面我们来分别进行介绍。以下所有图示及实现代码可以参见`Code/Chapter03/C19_Activation/main.py`文件。
...
$$
进一步，根据式(3-114)和式(3-115)我们可以分别画出两者的函数图像，如图3-50所示。
 
<div align=center>
<img width="350" src="statics/images/dl/23021828210.jpg"/>
</div>
...
在清楚Sigmoid激活函数的基本原理之后，我们再来介绍如何实现与使用。根据式(3-114)可知，Sigmoid的实现代码如下所示：
...
$$
g(x)=\frac{e^{x}-e^{-x}}{e^{x}+e^{-x}}\tag{3-116}
$$
其导数为
$$
g^{\prime}(x)=1-(g(x))^2\tag{3-117}
$$
进一步，根据式(3-116)和式(3-117)我们可以分别画出两者的函数图像，如图3-51所示。

<div align=center><img width="350" src="statics/images/dl/23021858281.jpg"/></div><center>图 3-51. Tanh函数图像</center>
...
在有了Sigmoid激活函数的实现示例后，Tanh就相对容易了。根据式(3-116)可知，Tanh的实现代码如下所示：
...
进一步，根据式(3-118)和式(3-119)我们可以分别画出两者的函数图像，如图3-52所示。

<div align=center>
<img width="350" src="statics/images/dl/240618110133.jpg"/>
</div>
...
根据式(3-118)可知，ReLU激活函数的实现代码如下所示：
...
x,\;\;&x\geq0\\[1ex]
\gamma x,& x<0
\end{cases}=\max(0,x)+\gamma \min(0,x)\tag{3-120}
$$
其导数为
$$
g^{\prime}(x)=\begin{cases}
...
进一步，根据式(3-120)和式(3-121)我们可以分别画出两者的函数图像，如图3-53所示。

<div align=center>
<img width="350" src="statics/images/dl/240618110233.jpg"/>
</div>
...
从图3-53可知，与ReLU激活函数的主要区别在于当$x < 0$时，LeakyReLU 仍旧存在一个较小激活值 $\gamma$ ，从而不会造成神经元的“死亡”现象。

**2\. 实现**

根据式(3-120)可知，LeakyReLU激活函数的实现代码如下所示：
...
在上述代码中，第2行便是式(3-120)的实现过程。第5~10行则是将其封装为一个网络层。
